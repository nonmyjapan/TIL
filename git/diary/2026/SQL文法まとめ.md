...
- 実行前にSELECTで対象を確認する。
- トランザクションを使用する（元に戻せるようにする）
🚨注意🚨
トランザクション途中で前に戻るとそのままコミットして消えてしまう！特にDELETE・UPDATEの時は要注意！
...
=
検索結果を指定した列の値で並び替える。
- ASC:昇順（小さい順 / A→Z / 古い順）昇りたいから小さい順
- DESC:降順（大きい順 / Z→A / 新しい順）降りたいから大きい順
```
-- 価格が高い順（降順）に並び替える
SELECT * 
FROM products 
...
OUTER JOIN（FORMAT）
=
OUTER JOIN（外部結合）は、2つのテーブルを結合する際、一方（または両方）のテーブルのデータを消さずに全件残して結合する構文です。INNER JOIN（内部結合）が「両方のテーブルに共通して存在するデータのみ」を取得するのに対し、OUTER JOIN は「データが存在しない側に NULL を補って表示する」のが大きな特徴です。
```

```
実務での活用シーン
- 「～していない人・データ」を抽出したいとき。
- マスターデータを欠かさず一覧表示したいとき。
...
=
IN 句と似ているが、大量のデータを扱う実務では EXISTS の方が処理速度が圧倒的に速いため重宝される。
「特定の条件に当てはまるデータが別テーブルに存在するか（しないか）」を判定する。
```
